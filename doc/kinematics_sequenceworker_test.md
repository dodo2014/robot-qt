...
依据：`doc/archive/gemini_qr.md`、`AGENTS.md` 防回归硬规约（测试台账 + 修改前验算）。
前置：**用户确认计划后再执行本方案**。
...
1. 验证「小脑」（Kinematics/CoordTransform/Home Offset）数学正确性与行为符合 `doc/archive/gemini_qr.md` 模型（**注意 L1 现行值为 138.83mm，非该档案中的 174.35mm**）。
...
| 编号 | 动作 | 期望结果 |
|---|---|---|
| H01 | J1 回零（仿真/真机） | 显示 **-102°**（逻辑=机械 0−offset）；J2 回零显示 **≈-28°**（真机受舵机重复精度影响可能 0.5~1° 偏差，允许 ±1°；仿真精确 -28） |
...
> **H01 真机实测（2026-08-21）**：J1 回零显示 -102° ✅；J2 显示 -27.4/-27.2/-27.3（机械停 0.6~0.7°，紧固金属套后复测）；R 显示 0.2°（系统性偏差，暂不处理）。**未回零位置显示**：卡轴逻辑=机械规划位(0)−offset（J1 显示 -102）；舵机=遥测真实机械角−offset（显示上电实际角度，非 0 属正常）。

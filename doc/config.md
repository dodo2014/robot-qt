...
└── safePos
    ...
    ├── r        0.0              安全位 R 关节角 (°)
    └── enabled  false            回零完成后自动回安全位开关（默认 false——出厂全 0 不应乱走）
```
...
- **UI 写入入口**：① 设备配置页·运动学参数区 4 个输入框 + 「回零后自动回安全位」checkbox；② 手动控制页「示教安全位」按钮（读当前四轴关节角写入并置 enabled=true，需已回零且轴静止）；③ 手动控制页「回安全位」按钮手动触发走位。

...
### 1. 全局行为准则
你是一个高级程序员 AI 助手。本项目是一个 **Qt + 机器人开发** 项目。
你必须保证所有的回答、生成的代码以及编写的文档（包括需求、测试用例、注释等）都极其专业，严禁使用机器翻译腔。

### 2. 核心领域规范（强制阅读）
本项目维护了一份详尽的领域术语和架构准则文档。
**⚠️ 强制指令**：在进行任何文档编写、代码生成或代码 review 之前，你必须首先读取并严格遵守同目录doc下的 `doc_output_standards_guide.md` 文件。

你可以使用你的文件读取工具，或者在上下文中自动包含它。如果你未能遵守 `doc_output_standards_guide.md` 中的《核心领域术语表》，我将会中断对话。
...
| 项目记忆与日志（五套，勿混用） | AGENTS.md / doc/worklog / .history / .workbuddy-ai / .codebuddy-plans 五处记忆的职责边界与关联规则 |
...
- **`.workbuddy-ai/`（WorkBuddy 项目级配置 + 记忆，2026-09-14 建立）**：项目级 WorkBuddy 配置根目录，含 `settings.json`（权限 + 5 条 hooks，入库共享）、`settings.local.json`（本机私有，gitignore）、`hooks/`（含 `guard-git.sh` 硬拦截 git commit/push）、`memory/`（MEMORY.md + 每日日志，gitignore）、`README.md`（目录说明 + 提交授权用法）。**提交授权令牌**：用户下指令后 `echo "指令原文" > .workbuddy-ai/.git-commit-authorized`（30 分钟 TTL，删文件即撤销）；令牌只能由用户创建。改 hooks 后跑 `bash .workbuddy-ai/hooks/_selftest.sh` 自检。
- **`.codebuddy/plans/`（CodeBuddy plan 模式产出，2026-09-14 纳入）**：**位置在用户级目录 `d:\dev\.codebuddy\`，不在本仓库内**（故不受 git 管理）。plan 模式生成的计划文档，随机文件名（`形容词-名词-科学家-随机串.md`）。**已核实 2026-09-14：17 篇全部为本项目相关**（2026-09-02 ~ 09-11），内容为带 `file:line` 的改动清单 + 验证/标定步骤 + 风险与回退，与 TEST_RECORD.md 的 TR 编号体系对应（例：`radiant-pulse-darwin-RI6lmv-r.md` = TR-090 接线 `MC_SetStopDec`）。**用途**：回溯"某项改动当时是怎么设计的、为什么这么设计"——worklog 记结果，plan 记设计意图与备选方案。**关联规则**：`projects/<项目slug>/<session-id>.jsonl`、`tasks/<session-id>/`、`file-history/<session-id>/` 三者**共用 session UUID**，可互相还原（对话全文 + 任务清单 + 文件改动）；**但 `plans/` 文件名不含 UUID，无法自动关联到会话**，只能靠内容或 mtime 推断。同目录还有 `history.jsonl`（提问历史，含 `project` 字段，可筛本项目）。**权威性**：plan 是**当时的计划**，不等于已落地——引用前须用 TEST_RECORD.md 核对实际状态（曾有 plan 落地后状态变更）。
...
1. **AutoRunPage** — 自动运行: camera views, LCD display, coord panel, log, 5 control buttons（2026-09-07 重构，详见 `doc/auto_run_button_refactor.md`）：▶启动/继续（Idle 启动新流程 / Paused 继续，启动受回零互锁）、⏸暂停（`Pause()` 点级暂停：减速停当前轴→挂起保持上下文）、■停止（`StopImmediate()`：cancel + 全轴减速停保持使能，废弃上下文）、↺清报警（`ClearFault()`：仅清引擎 Fault 锁存，不动硬件使能）、⟳初始化（`Initialize()`，按 未使能/未回零/就绪 分级引导去手动页；原「复位」实为 HomeAll，已归手动控制页）。**⟳初始化契约修正（2026-09-15，TR-093）**：`HardwareManager::Initialize()` 返回值语义 = **硬件是否真正就绪**（`IsMotionCardConnected() && IsServoConnected()`）——原实现**无条件 `return true`**，运动卡/舵机未连接也照样报「初始化完成 / 硬件已连接」，使 `OnInitClicked` 的失败分支成为**死代码**；同时 `initialized_` 原为一次性闩锁（`if (initialized_) return true;`），第二次点「初始化」直接返回、**根本不会重连**（与按钮 tooltip「重新加载配置并连接硬件」不符）。修后二次进入**只补做连接**（运动卡 `ConnectMotionCard()`、舵机 `ReconnectServos()`），故**接好硬件后再点一次「⟳ 初始化」即可恢复，不必重启程序**；失败提示带 `ConnectionStatus()`（如「运动卡: 未连接 | 舵机: 未连接」）并写日志框。⚠ `Initialize complete` 日志行**必须无条件打印**（Sim 冒烟以该行为通过判据），且 `initialized_` 仍置 true（对象与 50ms 轮询已建立，重入靠它判断走"补连接"分支）。**底部提示行已于 2026-09-15 迁出（TR-092）**：原 `m_hintLabel` 删除，`SetHint` 改为 `emit requestGlobalHint`，提示统一由 MainWindow 顶栏「全局消息横幅」承载（10 寸触控屏竖向空间优化）
2. **ManualControlPage** — 手动控制: enable/disable, 6-axis JOG table, 一键回零, 回安全位/示教安全位（2026-09-07，安全位详见 `doc/global_safe_position.md` 与 config `kinematics.safePos.*`）
3. **ProcessPage** — 工艺与流程: scheme management, action list (QListWidget), detail stack (QStackedWidget × 5 action types). Data stored in `process.json` via `ProcessManager`. 动作与方案的新增/编辑/删除/保存完整闭环，实时同步 JSON。
   - **单步执行 UI 现状（2026-09-03 真机验证通过，阶段 5）**：顶栏两行——第一行「当前方案: [名称框] [方案下拉] 新增方案 **切换方案**(原「确认切换」仅改文案) 删除方案」，第二行「**方案调试** | 单步执行 | 停止 | [状态标签]」（`ProcessPage.cpp:125-214`）。`m_statusLabel`（:192）接 `SequenceWorker::stateChanged`（**2026-09-07 起为 enum 签名 `(WorkerState, PauseReason)`**，原 QString 4 中文字符串已废）+ `schemeFinished`/`interrupted`/`errorOccurred` 显示状态——单步按钮可点的**权威条件 = Paused ∧ reason==Step**（比字符串判定更强，守 D2）；Running 分支按 `IsSingleActionSession()` 还原"● 执行选中动作"文案；用户暂停（Paused∧User）时单步按钮灰（NextStep 无效）。**此前 stateChanged 全项目 6 处 emit、0 处 connect，界面无任何执行状态反馈，2026-09-03 补齐接线**。实现要点：① 终态保留（StartExecution 在 schemeFinished/interrupted 后补发 `stateChanged(Idle)`，会用 `m_finalStatus/m_finalColor` 保留终态，新会话启动清空）；② 急停与业务停止都发 interrupted，靠 `IsGlobalEnabled()` 区分文案（急停断使能）。单步按钮**执行期同步禁用**（`OnStepExecute` 点击栈内即 `setEnabled(false)`，ProcessPage.cpp:1033/1042，非等信号）——D2：stepGo 是 bool 非计数，执行期点击会预置放行使下一个暂停点被跳过连跑两个动作，同步禁用把点击收敛到暂停期。切换/删除方案执行期门禁 `IsExecutionActive()`（:946 = m_stepActive || worker->IsRunning()，:774 删除、:814 切换，弹窗「单步执行尚未结束…」）。**2026-09-08 单步标志生命周期修正（TR-076）**：stepMode 清除点从 `RunSequence` 入口移到会话结束（`StartExecution` 末尾 + `RunSafePos` 显式双保险）——入口清除会抹掉 UI 刚设的单步标志，方案一次跑完（真机 8.13 实测）。**2026-09-09 执行选中动作互斥修正（TR-077）**：原 `OnRunSelectedAction` 在调用 `RunSingleAction` **前**清 `m_stepActive`，被 running 门禁拒后单步会话死锁——改为先判 `IsExecutionActive()` 门禁（拒绝时**不动状态**）+ `stateChanged` 联动 `m_runSelectedBtn` 置灰（Running/Paused 灰、Idle/Fault 恢复）+ `OnStepExecute` 防御（`m_stepActive` 与引擎态不同步即 Paused∧Step∧IsStepMode 时按"释放下一步"处理）。两类修复同一教训：**会话标志/引擎标志的清理必须放在生命周期末尾（操作成功确认之后），不能放在入口或调用之前**。
...
- **全局消息横幅（2026-09-15，TR-092）**: 自动运行页不再自绘提示，链路 = `AutoRunPage::SetHint` → `emit requestGlobalHint(text,color)` → `MainWindow::UpdateGlobalHint` → `HintBanner`（类定义在 `MainWindow.cpp`，实例 `globalHint_` 挂在顶栏【手动/自动】滑块正左侧，固定宽 320px、超长 `ElideRight` 截断、点击弹非模态全文框）。**手动控制页提示保留原地不动**（`ManualControlPage::hintLabel_` 与 `RefreshSoftLimitHint` 均未改），横幅**只接自动运行页**——单一信号源，规避 TR-087「单条覆盖式提示 + 多信号源 = 必然竞争」的放大。**四条硬约束（勿回退）**：① `HintBanner` 必须提为 `MainWindow` 成员——`CreateTopBar()` 在 `SetupUI` 中**先于子页面创建**被调用，局部变量在接线处不可引用；② 宽度用 `setFixedWidth(320)`——除限上界外，还经 `qSmartMinSize` 的 `boundedTo(maxSize)` 把 QLabel 的最小宽度一并夹住，否则长文案（急停指导约 60 字）的 `minimumSizeHint` 会把顶栏顶爆（顶栏余量：最小窗口 1200 时左区约 602px、标题占约 192px）；③ 点击弹窗用**非模态** `QMessageBox`（`setModal(false) + show() + WA_DeleteOnClose`）——静态 `QMessageBox::information()` 内部是 `exec()`，会阻塞主线程事件循环致遥测刷新（约 50ms 一次）与日志追加停摆；④ `elidedText` 必须在 `SetMessage` 与 `resizeEvent` **双点重算**，否则窗口拉伸后截断长度停留在上一次。**用 QLabel 派生而非 QPushButton**（全局 QSS 的 `QPushButton` 规则带 `padding:10px 18px / font-size:15px / border-radius:10px`，实例样式漏项即成大按钮）；字号走 `setFont` 而非 QSS，保证 `fontMetrics()` 与渲染一致（elide 精度依赖它）。初始文案由 `CreateTopBar` 给出（构造期信号在 `connect` 之前 emit 会被静默丢弃）。
...
- **XRServo 点动平滑 + 热重连（已完成）**：
  ...
  - **离线/重连抖动的硬件根因（2026-09-10 用户确认）**：COM 转 USB **转换器质量差、连接不稳定**（廉价型号）——舵机偶发离线（含急停 TorqueOff 后的短暂离线，TR-087）优先怀疑转换器硬件，非软件问题；软件侧热重连/降频/Ping 守卫均已就位。**待更换稳定型号转换器后复测**，届时抖动应显著下降。
...
- **2026-09-04 双修复/新功能（均真机验证通过 🟢）**：
  - **TR-070 舵机未上电误报"已连接"**：`IsServoConnected()` 收紧为「串口打开 且 所有已创建舵机 `IsOnline()`」（SimServo 的 IsOnline==connected_ 仿真不受影响）；`Initialize`/`ReconnectServos` 中 `SetServoId/SetSpeed` 移到 `Connect()` **之前**（Connect 内 Ping/WriteUserData(addr=44) 面向配置 id，旧时序 J2(id=0) 的在线标志错探成默认 id=1）。上电自动恢复走既有「Ping ok skip reconnect」+ PollTick 边沿广播，无需重启软件。已知开口：skip-reconnect 恢复路径不重写 addr=44 accel 参数（舵机用户数据预期掉电保持，点动顿挫回归再补写）。
  - **TR-071 示教记录关节角 + 确定性执行**：旧版 save 硬编码写 `joints{j1..j4}=0.0` 占位（真实关节角从未落盘）→ `PointData` 加 `hasJoints/j1~j4`，示教读取落盘真实关节角；执行 `MoveToPoint` 优先按 joints 直接运动（`Teach joints →` 日志，无 IK 双解歧义）。**⚠ 坐标语义（勿混淆）**：`joints.j3`=**Z 电机坐标**（回零后=0、往下为负，与手动页 Z 轴 Go 同坐标系）；`coord.z`=FK 末端高度（回零后=165），差固定偏移 Z0−h1−tcpDown=165，两者不是同一个数值，混用直接下发会撞顶。作废规则：表格手改坐标保存 → 该点 joints **删字段**（非置 0，0 是合法关节角）；执行层 FK 一致性校验（±0.5mm/0.5°）不匹配 → 回退 IK（拦截旧版全 0 占位残留，模板点不会误走到 J(0,0,0)）。posture 字段语义未动（见 SequenceWorker 已知改进项）。

- **2026-09-07 三项落地（编译+Sim 冒烟通过 🟢；GUI 交互与真机走位待统一真机验证——TR-075 互锁已落地，验证随之解锁）**：
  - **TR-073 自动运行页 5 按钮 + SequenceWorker 显式状态机**：4 钮 → 5 钮（▶启动/继续、⏸暂停、■停止、↺清报警、⟳初始化；原「复位」实为 HomeAll，已归手动控制页）；引擎 `WorkerState{Idle,Running,Paused,Fault}` + `PauseReason{None,User,Step}`，`stateChanged` 改 enum 签名（原 QString 中文字符串废，唯一消费者 ProcessPage 已同步）；**点级暂停+原地续走**（PauseGate：减速停当前轴→挂起→Resume 重发当前绝对目标 MoveAbs；Delay 按剩余时间续算；**回抽阶段暂停不反向补料**——"重试整个动作"的硬伤）；`StopImmediate()`=cancel+全轴减速停保持使能（自动页【停止】；`Stop()` 保持 cancel 就近供单步/退出，零回归）；Fault 锁存（失败且非 cancel，ClearFault 回 Idle 不动硬件使能）；**单步 stepMode 生命周期（TR-076 修正）**：`SetStepMode(true)`（ProcessPage）→ `RunSequence`（**不清**）→ 会话内每动作后 `Paused(Step)` 挂起 → **会话结束时清**（`StartExecution` 末尾；`RunSafePos` 另显式清一次双保险）——清除点放错位置会造成「单步失效（放开头）」或「新会话卡死 Paused(Step)（不清）」两种故障。**2026-09-08/09 阶段 8 真机验证通过（8.1–8.17 全 🟢，见 TR-078）**——过程中发现并修复 TR-076/TR-077 两个回归（均为"清除点位置错误"类）；8.4 引出循环生产新需求（`doc/auto_run_loop_production.md` 待审核）。详见 `doc/auto_run_button_refactor.md`。
  - **TR-074 全局安全位**：config `kinematics.safePos.{j1,j2,z,r,enabled}`（**关节角形态**，Z=电机坐标；enabled 默认 false——出厂全 0 不应乱走，示教后自动 true）；`RunSafePos()` 构造 2 点临时方案（原地抬Z→水平走安全位）复用 RunSequence——零新运动代码，白得暂停/停止/门禁/与自动运行互斥；回零完成 `homeStateChanged(true)` **上升沿**触发（MainWindow `lastHomed_` 判定——该信号是**电平语义**，手动页单轴重回零会重复 emit）自动回安全位 + 手动控制页「回安全位」「示教安全位」按钮（worker 已注入）；ConfigPage 运动学区 4 输入框 + 「回零后自动回安全位」checkbox（safePos 现读 config，**不 emit paramsChanged**）；ProcessPage actionStarted 加 `IsSafePosSession()` 守卫防列表误选。急停恢复链路闭环：急停→切手动→使能→一键回零→自动回安全位→切自动。**实现约束**：hasJoints 点必须 `impl_->kin.Forward` 同源填 x/y/z/r（±0.5 FK 校验，填 0 会静默回退 IK）。详见 `doc/global_safe_position.md`。
  - **TR-075 UI 权限与全局互锁（架构指令 2/2）**：**急停锁存 `estopPending_` 在 HardwareManager（全局硬件级状态，Gemini Q6，禁止页面自持副本）**——`EmergencyStop` 置位（须在 `emit enableStateChanged()` **之前**，否则 AutoRunPage 该信号槽一帧错误解锁），唯一清除条件=急停后全轴回零（MarkAxisHomed allHomed，**先清后发**专用信号 `estopCleared()`）；模式切换门禁=仅 Running/Paused 拦截（**Fault 放行**——无运动在执行，切手动脱困合法）；**单步会话 Q12 形态**：切自动→`ProcessPage::AbortStepSession()`（public 新增，Stop+ResetStepSession+状态文本）强制清理后放行，切手动放行；导航白名单 `{0 自动运行, 3 视觉检测}`（Gemini Q9），`VisionTestPage::SetParamsLocked` 锁 9 个参数控件（操作类保持可用）；`ApplyModeState` 为模式态唯一出口——**标签高亮从 toggled lambda 迁入**（拒绝回弹经 QSignalBlocker 屏蔽 toggled，高亮留 lambda 会残留被拒模式色，修正 ui_interlock_plan 5.5 结论）；强制跳页**双写** `setChecked+setCurrentIndex`（程序化置位不发 idClicked）；`m_schemeCombo->setEnabled(!busy)` 脱离 gate（手动/急停期保留预选）；弹窗文案区分 `IsSafePosSession()`（"正在返回安全位，请稍候"，不诱导停止）；手动页急停分步恢复指导（先 ResetAxisStates 再 SetHint 防被 RefreshSoftLimitHint 覆盖）+ `estopCleared` 绿字 + OnGlobalEnable 阶段化提示。详见 `doc/ui_interlock_plan.md` 实施定稿节。
...
- **接口（2026-09-07 重构，详见 `doc/auto_run_button_refactor.md`）**：**显式状态机** `enum class WorkerState { Idle, Running, Paused, Fault }` + `PauseReason { None, User, Step }`（Q_ENUM，跨线程 Queued 传递必需）；`RunSequence(const SchemeData&, const LoopConfig& loop = LoopConfig{})`（**默认参数 → 既有调用点零改动、行为零变化**；TR-091 循环生产：`LoopConfig{mode 单轮/Count/Infinite, count, returnToSafePos, retryCount=0, retryDelayMs}` 值语义、主线程组装后进 worker 线程，worker 内**绝不读 config**；使能+回零+running 门禁，未使能拒绝+errorOccurred；**⚠ 不清 stepMode**——单步由 UI 先 `SetStepMode(true)` 再调本函数，入口清除会抹掉单步标志致方案一次跑完，2026-09-08 真机 8.13 实测，见 TR-076；stepMode 清除点在**会话结束时** `StartExecution`/`RunSingleAction`）/ **`RunSingleAction(const SchemeData&, int actionIndex)`（单动作独立执行，共用 running 门禁互斥；强制清 stepMode）** /`Stop()`（**仅置 cancel，当前动作执行完才停**，保持使能——单步/ShutdownWorker 用）/ **`StopImmediate()`（cancel + `StopAllAxes()` 全轴减速停保持使能，自动页【停止】用）** /`EmergencyStop()`（+断使能）/ **`Pause()/Resume()`（点级暂停：在挂起点减速停当前轴→挂起保持上下文→Resume 重发当前绝对目标 MoveAbs 原地续走；Delay 按剩余时间续算，回抽阶段不反向补料）** / **`ClearFault()`（Fault 锁存→Idle，不动硬件使能）** / **`RunSafePos()`（全局安全位，见 Current Phase 节 TR-074）** /`SetStepMode(bool)`/`NextStep()`；查询 `IsRunning()=state!=Idle`（单步暂停时亦 true，历史语义零回归）/`IsPaused()=state==Paused`/`IsStepMode()`/`GetState()/GetPauseReason()/IsSingleActionSession()/IsStepGatePending()/IsSafePosSession()`；信号 `actionStarted/actionFinished/`**`singleActionFinished`**`/schemeFinished/interrupted/errorOccurred/logMessage/`**`stateChanged(WorkerState, PauseReason)`**（原 QString 4 中文字符串已废，唯一消费者 ProcessPage 已同步改 enum）/ **`cycleChanged(int index, int total)`**（TR-091 循环进度：index 从 1 起、total<0 = 无限循环；每轮开始时发一次）。
- **单步语义（N+1 次点击，勿按 N 次预期）**：单步 = `SetStepMode(true)` + `RunSequence`，与连续运行共用同一 `ExecuteActions()` 主循环，唯一分歧——每动作 `ExecuteAction` 成功后 `stepGatePending=true` + `SetState(Paused, Step)`（原字符串"单步暂停"已废）→ `WaitForStep()`（20ms 轮询 `cancel||stepGo` 的 QEventLoop，退出时 `stepGo.store(false)`、`stepGatePending=false`）→ `NextStep()`/`Resume()` 置 `stepGo=true` 唤醒。**`WaitForStep` 在 for 循环体内：最后一个动作执行完也会暂停**，须再点一次才退出循环发 `schemeFinished` → **N 个动作需 1 次启动 + N 次释放 = N+1 次点击**（引擎层自测 S04「5 动作 5 次 NextStep」即含启动首点）。`NextStep()` 返回 false（非 running/非 stepMode）时 UI 不应置灰按钮；UI 侧单步按钮可点权威条件 = `GetState()==Paused && GetPauseReason()==Step`。
...
- **动作实现**：Move=**优先示教 joints**（`pt.hasJoints` 且 FK 一致性校验 ±0.5mm/0.5° 通过 → 直接按关节角下发，日志 `Teach joints →`，确定性复现示教姿态；校验失败 WARN 回退 IK——TR-071，坐标语义见 Current Phase 节），否则 `InverseSmart`（TCP 已内化）→ 逐轴 `MoveAbs`（先 J2/R 舵机后 J1/Z 卡轴）→ `WaitForAxes` 轮询 `IsAxisBusy` 到位（30s 兜底）；Vision=SimCamera 采帧+SimAlgo 检测+`CoordTransform::CameraToRobot` 手眼换算（无相机/算法时模拟延时）；Extrude=挤出量/回抽量（绝对目标=挤出量−回抽量，Extruder 限位 [0,100] 恒正）；Delay=`QEventLoop`+20ms 轮询可被 cancel 打断；Gripper=取 `action.gripperTarget`（轴5 绝对目标 mm，0=夹紧/负=松开），`IsWithinSoftLimits` 越界拒绝（不静默夹紧），速度=`GetMaxSpeed(Gripper)×speedPercent`，到位超时按 `GetAxisBusyMs` 动态兜底（2026-09-02 重写，旧版 `isGripperOpen?GetLimitMax:GetLimitMin` 与物理方向相反已废弃）。
- **中断/暂停/故障语义（2026-09-07 重构）**：`cancel` 原子标志 + 等待循环（`WaitForCancelOrTime`/`WaitForAxes`/`WaitForStep`/`PauseGate`）20ms 轮询退出 → `ExecuteActions` 检测后发 `interrupted("用户停止")`。**三级停止**：① `Stop()` 仅置 cancel（跑完当前动作才停，TR-063 实测——单步/ShutdownWorker 用）；② `StopImmediate()` = cancel + `StopAllAxes()` 全轴减速停保持使能（MC_Stop 斜坡/舵机锁位，非断脉冲——开环断脉冲会丢步；自动页【停止】用）；③ `EmergencyStop()` = cancel + 硬件断使能（真刹车）。**暂停** = `Pause()` 协作式标志，在最近挂起点（`WaitForAxes`/`PauseGate`/点边界/动作边界，≤20ms）`StopAxes` 减速停后挂起，`Resume()` 重发当前绝对目标 MoveAbs 原地续走（Delay 按剩余时间续算；回抽阶段暂停不反向补料）。**故障锁存**：动作失败且非 cancel → `SetState(Fault)` 保留上下文，`RunSequence` 被拒直至 `ClearFault()`（仅清引擎，不动硬件使能——卡轴清驱动报警靠手动页断使能+重使能）。
...
- **自测**：临时驱动 17/17 通过（2026-08-20，仿真 Sim 全家桶）：S01 完整方案 5 动作、S03 Vision 闭环（基座 8.6,-0.8,55.0 conf=1.00）、S04 单步 5 次 NextStep、S05 Stop、S06 EmergencyStop、S07 未使能拒绝、S08 线程不卡。测试驱动已清理。2026-09-07 重构（TR-073/TR-074）：Debug+Release 双编译 + Sim 冒烟通过；暂停/继续/停止/清报警/安全位走位的 GUI 交互与真机走位**待权限互锁（架构指令 2/2）完成后统一真机验证**（用户指令）。
...
- **已知改进项（2026-08-31 用户指出，2026-09-04 TR-071 部分覆盖）**：`MoveToPoint` 曾一律用 `InverseSmart`（双解就近）求解、无视示教姿态——**对纯示教点已由 joints 确定性执行解决（TR-071）**。剩余：① 示教按 `cur.j2` 符号记录 posture（≥0→elbow_up / <0→elbow_down，当前仍硬编码 elbow_up）；② **手改过坐标的回退点**执行按 posture 用 `Inverse(target, out, elbowUp)` 确定性解，`InverseSmart` 仅作 posture 缺失/未知兜底。触发背景：点_002 报 `IK(Smart)` 时用户质疑"elbow 已确认为何还走 ik smart"（当时直接根因是 R 限位域不一致，见 XRServo 节）。
...
**下一步**（历史计划，T7–T10 已完成；按钮映射已被 2026-09-07 5 按钮重构取代，现行见 `doc/auto_run_button_refactor.md`）：~~AutoRunPage 5 按钮接线（启动=运行页内方案下拉选中方案 / 复位=HomeAll / 停止=Stop / 初始化=Initialize / 急停=EmergencyStop）~~ → 现行：▶启动/继续=RunSequence+Resume、⏸暂停=Pause、■停止=StopImmediate、↺清报警=ClearFault、⟳初始化=Initialize（不含回零）。T9 示教读取、T10 参数接入、相机接入 frameReady 等其余项已完成或进行中；奥比中光（Orbbec）真实相机 SDK 实现 `ICamera` 待同事提供 SDK 后接入。自动流程中 `PickCycleController` 视觉抓取单周期模板后续作为 SequenceWorker Vision 动作的委托实现。
...
- **T8 AutoRunPage**（2026-08-20 原始接线，**按钮组已被 2026-09-07 5 按钮重构取代，现行见 `doc/auto_run_button_refactor.md`**）：方案下拉（选中方案传给 RunSequence）/按钮组（原：启动/复位=HomeAll/停止=Stop/初始化=Initialize/急停=EmergencyStop → 现：启动/继续、暂停、停止=StopImmediate、清报警=ClearFault、初始化=Initialize）/坐标面板 FK 实时刷新/日志框接 logMessage/双相机 QLabel 接 frameReady。
...
- **按钮变化**：AutoRunPage 5→4（2026-08-28 删"⚔ 急停"）→ 5（2026-09-07 重构：▶启动/继续、⏸暂停、■停止、↺清报警、⟳初始化，详见 `doc/auto_run_button_refactor.md`）；ManualControlPage 顶部 4→3（使能/断使能/一键回零）→ 5（2026-09-07 增「回安全位」「示教安全位」）；ProcessPage「单步执行」右侧红色「停止」已接 `SequenceWorker::Stop()`（业务停止就近，经 `SetSequenceWorker` 注入 worker，与 AutoRunPage 同模式；**2026-09-07 起自动页【停止】改走 `StopImmediate()`，`Stop()` 仅剩单步停止与 ShutdownWorker 使用**）；ProcessPage「单步执行」经 `SetStepMode(true)+RunSequence` / `NextStep` 接单步模式（此前为 stub，本轮补齐）。
- **语义区分**：停止=业务（可恢复、保持使能）、急停=安全（断使能、需重新使能）。~~TEST_RECORD 记 🟡，阶段 5 用例 5.6/5.7 覆盖真机验证~~ **已收口（TR-040，2026-09-04：5.4 + 5.12 两项均真机验证通过）**。
...
**下一步**：真机联调**阶段 4 重做修复已全部真机复测通过（2026-08-28 🟢）**；舵机重连抖动治理真机观察中（硬件定性见上）。SequenceWorker 方案真机验证**阶段 5 全部通过（2026-09-03 单步 5.1-5.5 + 2026-09-04 自动运行 5.6-5.15，TR-041/TR-040/TR-069）**；2026-09-04 追加两项真机通过收口（TR-070 舵机连接状态语义 + 探测 id 时序、TR-071 示教关节角落盘 + 确定性执行，均见 Current Phase 节），待做**阶段 6（AutoRunPage UI 接线真机）与阶段 7（异常/回归）**。待补：**三页操作互斥**（手动/工艺流程/自动运行，2026-09-03 用户确认后续补充，TR-066）；**5.9 真实相机未接走降级路径，接相机后需复测**（Orbbec SDK 待同事提供后接入 `ICamera`）。
...
- **phase3 三阶段真机验证全部收口（2026-09-08/10，里程碑 🟢）**：阶段 8（TR-073，8.1–8.17）/ 阶段 9（TR-074，9.1–9.17）/ 阶段 10（TR-075，10.1–10.20）真机全部通过；**急停恢复闭环 = 使能→回零→手动回安全位→切自动**（TR-081 修正版，恢复链不再自动回安全位——自动回安全位仅**启动后首次**回零触发，上升沿语义）。过程产出 **TR-076~TR-089 共 14 条**：单步回归×2（TR-076/077）、配置页示教同步+数值量化（TR-079）、回安全位误报「有任务执行中」（TR-080，舵机重复回零竞态+Fault 误报）、上升沿语义澄清+急停恢复链改手动（TR-081）、`safePos.enabled` 总开关语义改名「启用安全位」（TR-082）、一键回零终止残留单步会话（TR-083，Paused 终止/Running 拒绝/Fault 预告）、`QPointer` 根治退出悬垂崩溃（TR-084，4 处观察者指针）、停止曲线答疑（TR-085，点动与暂停同用 `MC_Stop` 斜坡）、10.12 用例不可达溯源（TR-086）、急停提示被重连提示覆盖守卫（TR-087）、绿字③锁存+安全位完成态特判（TR-088）。**硬件环境**：舵机偶发离线根因=COM 转 USB 转换器不稳定（廉价型号，待换型复测，TR-087 备注）。详见 `TEST_RECORD.md` TR-076~089 与 `doc/worklog/2026-09-08/09/10.md`。**下一批次：循环生产真机验证**（`doc/auto_run_loop_production.md`，用例**追加进 phase3 编号体系** = 8.18–8.26 / 10.21–10.26，phase3 现最大 8.17 / 10.20 故无撞号——2026-09-14 更正原「独立编号 10.1–10.26 撞号」表述；方案文档同日已对齐 09-09/09-10 现状：行号校正、TR 编号改 TR-091 起、补 TR-083/088/090 事实）。**循环生产已实施并真机验证通过（2026-09-14 实施 TR-091，2026-09-15 验证 TR-094 🟢）**：自动运行页新增「循环」控件行（模式 单轮/指定次数/无限循环 + 次数 1~9999 + 「回安全位」复选框 + 进度标签，**一行排布、最终布局待真机效果后定**，硬约束 10 寸触控屏 + 最小窗口不错乱）；引擎在**会话内**加循环外壳（`ExecuteActions` 改循环壳、原主体原样搬迁为 `ExecuteOnce`），`RunSequence(scheme, loop = LoopConfig{})` 用**默认参数**→既有 3 个调用点零改动；每轮发 `cycleChanged`，`schemeFinished` 仍只在**全部循环完成**时发一次；循环间隔**内联**回安全位（与手动回安全位共用 `BuildSafePosActionFrom` 构造与执行链，故 8.20 的核验对 9.6 同样成立）；`production.loop.*` 持久化（不 emit paramsChanged）；`main.cpp` 启动清理 7 天前日志（S8，首次运行实测删 29 个过期文件）。**同时落地两项加固**：① `errorOccurred` **单点化**（D17 方案 A——`ExecuteAction` 子树 8 处 emit 改为写 `Impl::lastError`，emit 收敛到 **2 处**会话级出口，顺带修掉"具体失败原因被 `action.name` 覆盖"的既有瑕疵）；② 「严格先抬 Z」**ε 跳过下发**（D19——新增匿名 namespace 函数 `DispatchPointMove()`：目标与读回差 ≤0.01 的轴跳过并写 `SPDLOG_INFO skip axis X (in position)`，跳过前仍过 `IsWithinSoftLimits` 不旁路软限位硬拦截；`reissue` 复用同函数）。`MainWindow`/`ProcessPage`/`ManualControlPage`/`HardwareManager` **零改动**；Debug+Release 双编译 EXITCODE=0 + Sim 冒烟 `SMOKE_INIT_COMPLETE=True`。**循环生产真机验证收口（2026-09-15，TR-094）**：phase3 第四批 8.18–8.26 / 10.21–10.26 执行完毕——8.18–8.26、10.21–10.23、10.26 全 🟢（含 8.20 ★核心「先抬 Z」`skip axis` 核验、10.23 循环中急停恢复链仍成立、10.26 单次和多次循环后正常完成）；**10.24/10.25 ⚪ 用例不可达**（自动模式导航白名单 `{自动运行, 视觉检测}` → 工艺流程页/设备配置页置灰 + 模式互锁拒切手动 → 操作路径物理不存在，同 10.12/TR-086 根因族、与 D14 同源，用户裁决不单开 TR）。实测记录未逐条留存日志串（用户口头确认通过，phase3 实测记录块已如实注明）。**至此 phase3 全部批次（阶段 8/9/10 + 循环生产第四批）收口**。
